 ...
 * @author guregodevo
 */
object AuthorGephiDataSource extends GephiDataSource[ItemAuthorData, AuthorVertex, AuthorEdge] {
  ...
  override def parseNode(xmlBlock: String, processors: Array[MetricProcessor[_,GraphVertex]]): (Long, AuthorVertex) = {
    ...
    val node = new AuthorVertex(attrs.getOrElse(NAME, null), attrs(ACTIVITY).toInt, attrs(RATING).toInt, attrs.getOrElse(COUNTRY, null), attrs.getOrElse(LOCATION, null),
      attrs.getOrElse(WEBSITE, null), parseGephiDate(attrs(FIRSTDATE)), parseGephiDate(attrs(LASTDATE)), attrs(URLS).toInt, attrs(DOMAINS).toInt)
...
class AuthorGephiDataSourceFactory extends ServiceFactory[GephiDataSource[ItemAuthorData, AuthorVertex, AuthorEdge]] {
  override def service(params:NetworkParameters) = AuthorGephiDataSource
}

...
import com.grafalgo.graph.spi.util.spark.ForkedPregel
 ...
 */
class HighBetweennessCore(var hbseConf: HBSEConf, private var previousPivots: mutable.Set[VertexId]) extends Logging with Serializable {
   ...
   */
  def this(p:NetworkParameters) = this(new HBSEConf(p), new mutable.HashSet[VertexId])
   ...
   */
  def runHighBetweennessSetExtraction[VD <: GraphVertex, ED <: GraphEdge](sc: SparkContext, graph: Graph[VD, ED])(implicit tag: ClassTag[VD], etag: ClassTag[ED]): (RDD[(Long, Double)], Graph[HBSEData, Long]) = {
    ...
    var totalNumberOfVertices = hbseGraph.vertices.count()
    do {
      ...
      pivots = selectPivots(sc, hbseGraph, totalNumberOfVertices) //TODO check randomness. Check pivots are not already chosen. Use a bitset to mark them 
      ...
      logInfo("Number of pivots selected " + numberOfPivotsSelected)
  ...
  private def shouldKeepRunning(delta: Int, numberOfPivotsSelected: Int): Boolean = {
    if (hbseConf.maxtotalNumberOfPivots < numberOfPivotsSelected) {
      false
    }
  ...
  private def shortestPathRun(graph: Graph[HBSEData, Long], pivots: Broadcast[mutable.Set[VertexId]]): Graph[HBSEData, Long] = {
    ...
    do {
      ...
      def mapShortestPathMessage(msg:VertexRDD[mutable.Map[Long, ShortestPathList]]) = msg.distinct() 
      
      val prevG = hbseGraph
      hbseGraph = ForkedPregel(hbseGraph, mutable.Map.empty[Long, PathData], maxIterations = 20, activeDirection = EdgeDirection.Out)(accumulatePathData, sendShortestPathMessage, mergeMapMessage).cache()
   ...
   */
  private def selectPivots(sc: SparkContext, hbseGraph: Graph[HBSEData, Long], totalNumberOfVertices:Long) = {
    ...
    while (pivots.size < hbseConf.pivotBatchSize && totalNumberOfPivotsUsed < totalNumberOfVertices) {
   ...
   */
  private def createHBSEGraph[VD <: GraphVertex, ED <: GraphEdge](graph: Graph[VD, ED])(implicit tag: ClassTag[VD], etag: ClassTag[ED]): Graph[HBSEData, Long] = {
    graph.mapVertices((vid, vd) => new HBSEData()).mapEdges(e => if (e.attr.weight == 0) 1L else e.attr.weight.toLong)
  }

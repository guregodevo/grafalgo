...
/**
 * ReImplements a Pregel-like bulk-synchronous message-passing API.
 * 
 * Note : Unlike the GraphX Pregel API,, the Forked Pregel API weakens the assumption that sendMessage computation always return unique messages. 
 * To avoid duplicates, an additional step takes care of getting rid off possible duplicates that come up after the message sending computation.  
 *
 ...
 *
 */
object ForkedPregel extends Logging {
  ...
  def apply[VD: ClassTag, ED: ClassTag, A: ClassTag]
     (graph: Graph[VD, ED],
      //mapMsgRDD:  VertexRDD[A] => RDD[(VertexId, A)],
      initialMsg: A,
    ...
    : Graph[VD, ED] =
  {
    ...
    while (activeMessages > 0 && i < maxIterations) {
      ...
      prevG = g
      g = g.joinVertices(messages.distinct())(vprog).cache()
      ...
      logInfo("Pregel Message count " + activeMessages)
